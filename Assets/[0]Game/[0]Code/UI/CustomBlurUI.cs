...
namespace Game
{
    ...
    public class CustomBlurUI : MonoBehaviour
    {
        ...
        [SerializeField]
        private int _devisionSize = 7;
        
        [SerializeField]
        private bool _showOnEnable;
        
        [SerializeField]
        private GameObject[] _noRendererGameObjects;
        ...
        private void OnEnable()
        {
            if (_showOnEnable)
            {
                if (_noRendererGameObjects.Length != 0)
                {
                    foreach (var noRenderer in _noRendererGameObjects) 
                        noRenderer.SetActive(false);
                }
        ...
        private IEnumerator IEShow()
        {
            ...
            yield return ScreenShooter.IESpriteCamera(_devisionSize, x =>
            {
                Image.sprite = x;
                Image.enabled = true;
                texture = x.texture;
